class Calculator{
    ...
    generateResult(){
        if(this.getLastInputType() === "number") {
            const self  = this;
            const simplifyExpresssion = function (currentExpression , operator){
                if(currentExpression.indexOf(operator) === -1) {
                    ...
                    let partialSolution = self.performOperaton(...currentExpression.slice(leftOperandIdx,rightOperandIdx+1));
                    currentExpression.splice(leftOperandIdx,3,partialSolution.toString());

                    return simplifyExpresssion(currentExpression,operator);
                }
            }
            let result = ["×", "+" , "-","÷"].reduce(simplifyExpresssion,this.getAllInputValues());
    ...
    performOperaton(leftOperand , operation , rightOperand){
